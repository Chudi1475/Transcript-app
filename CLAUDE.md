...
- **Local transcription:** `faster-whisper` (`large-v3`, CUDA, `int8_float16`, beam 5, **sequential**, sensitive VAD). OOM-safe load ladder falls back `large-v3 int8` → `distil-large-v3` → `small.en` → CPU so it always starts. Long audio (>= `WHISPER_LONG_AUDIO_SEC`, default 480s) auto-switches to the batched pipeline at `WHISPER_FAST_BEAM_SIZE` (default 1) — several times realtime instead of ~1x, at the cost of coarser (~30s) segments; short clips keep the sequential max-accuracy path. Knobs: `WHISPER_MODEL`, `WHISPER_BEAM_SIZE`, `WHISPER_VAD_THRESHOLD` (default 0.2), `WHISPER_BATCH_SIZE`, and `WHISPER_BATCHED=1` to force batched for every job.
...
- Chromium MV3 extension: content script + a background service worker that relays the localhost health probe (a content-script fetch to localhost counts as the page touching the local network — Brave silently blocks it and Chrome permission-gates it, so probing directly always reported the PC dead)
...
- **Groq 25MB cap.** Long videos (>~25 min) won't go through cloud. When a cloud job dies on size, the frontend now offers a "Run it on this PC instead" link — and hands the URL off to `localhost:8000` automatically if the local server answers a health probe. On mobile it shows a hint instead (localhost on a phone is the phone). The extension probes local through its background service worker (see Tech stack — direct content-script probes are policy-blocked and stranded everything on cloud), 2 attempts × 1500ms.
...
- **Render cold start.** Free dynos sleep after 15min idle; the first hit then waits ~30-60s. Primary fix: the cloud app **keeps itself awake** — a self-ping thread in `cloud/app.py` hits its own public `/healthz` every 10min (`RENDER_EXTERNAL_URL`, auto-set by Render), during 7am-1am Central (`KEEPALIVE_UTC_HOURS`, hours in UTC) so idle nights don't burn the 750 free instance-hours/month. The GitHub Actions workflow (`.github/workflows/keep-alive.yml`) is only the resurrection knock for after quiet hours or a Render restart — GitHub dequeues `*/5` crons every 1.5-2.5 **hours** in practice, so it can't be the primary keep-warm. The extension also pre-warms the cloud the moment its input is focused. Expect one cold start on the first morning use (if nothing woke it since 1am), warm all day after.
